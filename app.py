...
T = {
    "English": {
        ...
        "no_key": "⚠️ OPENAI_API_KEY not set in Streamlit secrets.",
...
**Languages:** English · हिन्दी · ಕನ್ನಡ
    ...
    "हिन्दी": {
        ...
        "no_key": "⚠️ OPENAI_API_KEY सेट नहीं है।",
...
- Grok-3-mini LLM दोनों को मिलाकर सलाह देता है
    ...
    "ಕನ್ನಡ": {
        ...
        "no_key": "⚠️ OPENAI_API_KEY ಸೆಟ್ ಆಗಿಲ್ಲ.",
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown('<p style="color:var(--muted);font-size:0.75rem;font-weight:600;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:0.4rem;">🌐 Language / भाषा / ಭಾಷೆ</p>', unsafe_allow_html=True)
    for _lbl, _key in [("🇬🇧  English","English"),("🇮🇳  हिन्दी","हिन्दी"),("🌿  ಕನ್ನಡ","ಕನ್ನಡ")]:
        _active = st.session_state.get("lang","English") == _key
        if st.button(_lbl, key=f"sb_lang_{_key}",
                     type="primary" if _active else "secondary",
                     use_container_width=True):
            st.session_state["lang"] = _key
            st.rerun()
    st.markdown("---")
    st.markdown('<p style="color:var(--muted);font-size:0.8rem;">🔗 Data: AGMARKNET + data.gov.in<br>🤖 LLM: gpt-4o-mini (OpenAI)<br>🗄️ Vector DB: FAISS</p>', unsafe_allow_html=True)

# Resolve active language from session state
...
# ── Language pill tab bar at top of main area ─────────────────
st.markdown("""
<style>
.lang-pill button {
...
_lc1, _lc2, _lc3, _lc_rest = st.columns([1.2, 1.4, 1.4, 6])
for _col, (_lbl, _key) in zip([_lc1, _lc2, _lc3],
        [("🇬🇧 EN", "English"), ("🇮🇳 हिन्दी", "हिन्दी"), ("🌿 ಕನ್ನಡ", "ಕನ್ನಡ")]):
    with _col:
        st.markdown('<div class="lang-pill">', unsafe_allow_html=True)
        if st.button(_lbl, key=f"top_lang_{_key}",
                     type="primary" if lang == _key else "secondary",
                     use_container_width=True):
            st.session_state["lang"] = _key
            st.rerun()
        ...
        if not ready:
            ...
            if st.button(tx["predict_btn"], key="btn_predict"):
                ...
                with st.spinner(tx["spinner_predict"]):
                    result_text, retrieved_docs = rag_query(
        ...
        if not n_ready:
            ...
            if st.button(tx["evaluate_btn"], key="btn_evaluate"):
                with st.spinner(tx["spinner_eval"]):
                    from rag_engine import retrieve, load_index, get_client
                    import json
                    _index, _docs = load_index()
                    _client = get_client()
                    query = f"{n_commodity} {n_variety} {n_grade} price {n_market} {n_district} {n_state}"
                    retrieved = retrieve(query, _index, _docs)
                    context = "\n".join(f"• {d}" for d in retrieved[:5])
                    eval_prompt = f"""Based on these mandi prices for {n_commodity}:
{context}

Farmer offers Rs.{int(offered_price):,} per quintal.
Reply ONLY with valid JSON (no markdown):
{{
  "estimated_fair_price": <integer>,
  "verdict": "FAIR" | "LOW" | "HIGH",
  "confidence_pct": <0-100>,
  "advice": "<one sentence in {lang}>"
}}"""
                    ...
                    _raw = _resp.choices[0].message.content.strip().replace("```json","").replace("```","").strip()
                    try:
                        result = json.loads(_raw)
                    except Exception:
                        result = {{"estimated_fair_price": int(offered_price), "verdict": "UNKNOWN", "confidence_pct": 50, "advice": _raw}}
...
with tab3:
    ...
    with col2:
        st.metric("OpenAI API", "✅ Key Set" if has_api_key() else "❌ Not Set")
    with col3:
